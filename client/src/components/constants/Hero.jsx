        ...
        <div className="container flex flex-col px-6 py-4 mx-auto space-y-6 h-screen md:h-128 md:py-4 md:flex-row md:items-center md:space-x-6">
          ...
          >
            <div className="flex justify-center order-2 mt-6 md:mt-0 md:space-y-3 md:flex-col">
              <button className="w-3 h-3 mx-2 bg-yellow-500 rounded-full md:mx-0 focus:outline-none" onClick={changeImage}></button>
              <button className="w-3 h-3 mx-2 bg-yellow-300 rounded-full md:mx-0 focus:outline-none hover:bg-yellow-500" onClick={changeImage}></button>
              <button className="w-3 h-3 mx-2 bg-yellow-300 rounded-full md:mx-0 focus:outline-none hover:bg-yellow-500" onClick={changeImage}></button>
              <button className="w-3 h-3 mx-2 bg-yellow-300 rounded-full md:mx-0 focus:outline-none hover:bg-yellow-500" onClick={changeImage}></button>
            </div>

            <div className="max-w-lg md:mx-12 md:order-2">
              <h1 className="text-3xl font-kanit font-medium tracking-wide text-gray-800 dark:text-white md:text-6xl">
                Discover Tunisia's Beauty on Wheels with TunisianAutoTrek
              </h1>
              <p className="mt-4 text-gray-600 dark:text-gray-300">
                Uncover Tunisia's wonders on your terms with{' '}
                <span className="text-yellow-500 font-bold capitalize">TunisianAutoTrek</span>'s premium rental cars.
              </p>
              <div className="mt-6">
                <a
                  href={docForm}
